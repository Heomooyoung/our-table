...
(async()=>{
 ...
 ok(st.includes('모두 ')&&st.includes('우리테이블'),'개수와 출처가 꼬리에 붙음');
 ...
 ok(!!q('#scr .glogo'),'첫 화면에 로고');
 ok(!!byText('#scr .gname','우리테이블'),'앱 이름이 우리테이블');
 ok(!byText('#scr','우리집 식탁'),'옛 이름은 화면에 없음');
 ok(!!byText('#scr .gsub','오늘 뭐 먹지'),'무슨 앱인지 한 줄로');
 ok(!!q('#scr .gacts .kko')&&!!q('#scr .gacts .ggl'),'카카오·구글 버튼이 각자 색을 가짐');
 ...
 ok(!!byText('#scr .gsub','가족이 초대했어요'),'초대받은 티가 나는 첫 화면');
 ok(!!byText('#scr .gfoot','입력할 필요 없어요'),'코드 입력 안내 대신 "로그인만 하면 됨"');
 ...
 console.log('\n[18-4단계] 둘러보기는 구경만 — 쓰기 차단·서버 미기록');
 g('(function(){ui.peek=true;ui.gate=false;ui.tab="menus";render()})()'); await sleep(120);
 ok(!!byText('.demobar','저장되지 않아요'),'둘러보기 중임을 알려줌');
 ok(!!q('.peekbar[data-a="startNow"]'),'로그인하러 나가는 버튼이 있음');
 ok(!!byText('.peekbar .pk','로그인하러 가기'),'버튼 문구가 명확함');
 const peekBefore=g('S.menus.length');
 g('ACT.newMenu&&document.querySelector(\'.fab\')&&document.querySelector(\'.fab\').click()'); await sleep(150);
 ok(g('SHEETS.length')===0,'메뉴 등록 시트가 열리지 않음');
 ok(toast().includes('구경만'),'왜 안 되는지 알려줌');
 ok(g('S.menus.length')===peekBefore,'아무것도 추가되지 않음');
 ok(g('peekBlocked("saveMenu")')===true,'저장류 동작은 막힘');
 ok(g('peekBlocked("go")')===false,'화면 넘겨보기는 허용');
 ok(g('peekBlocked("detail")')===false,'메뉴 열어보기도 허용');
 ok(g('peekBlocked("startNow")')===false,'나가기 버튼은 허용');
 // 서버 기록도 남기지 않는다
 g(`(function(){window.__logged=[];sb={from:()=>({insert:o=>{window.__logged.push(o);return{then:(a,b)=>{}}}})}})()`);
 g('logVisit("menu_add","테스트")'); await sleep(60);
 ok(g('window.__logged.length')===0,'둘러보기 중에는 접속 로그도 안 남음');
